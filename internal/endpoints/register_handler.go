...
func (h *Handler) RegisterHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !isValidUsername(req.Username) {
		RespondWithError(w, http.StatusBadRequest, "Invalid username format", nil)
		return
	}
	...
	if err == nil {
		RespondWithError(w, http.StatusConflict, "Registration Failed", nil)
		return
	}
	...
	// Reason why I use formatting and utc parts is sqlite only accepts primitive forms as inputs.
	// Good for tweaking around but pain in the ** to format all there time modules.
	now := time.Now().UTC()
